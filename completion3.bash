...
__medium_debug_func_entry() {
    ...
    __medium_debug "$red$1:$reset ${green}c=$c$reset, words=$blue[${wordscopy[@]}]$reset, cur=$cur, cword=$cword, prev=$prev"
}

__medium_debug_command_state() {
  local red='\e[31;1m' reset='\e[0m'
     __medium_debug "$red$1:$reset
  commands = [${commands[@]}]
...
__medium_debug_compreply() {
    local red='\e[31;1m' green='\e[32;1m' reset='\e[0m'
    __medium_debug "$red$1:$reset COMPREPLY=$green[${COMPREPLY[@]}]$reset\n"
}
...
__medium_handle_reply()
{
    ...
    case $cur in
        -*)
            ...
            __medium_debug_compreply "${FUNCNAME[0]}"
            return 0;
            ;;
    esac
...
_medium_root_command()
{
    ...
    if [[ -z "${BASH_VERSION}" || "${BASH_VERSINFO[0]}" -gt 3 ]]; then
        command_aliases+=("dr")
        aliashash["dr"]="draft"
        command_aliases+=("drft")
        aliashash["drft"]="draft"
    fi
